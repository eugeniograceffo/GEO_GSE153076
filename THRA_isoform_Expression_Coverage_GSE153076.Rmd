...
## Lets separate the dataset: one for gene expression (to be normalized), one for A1vsA2 ratios (no normaliz)

dataset_gene <- select(dataset, -"9a", -"9b")
dataset_isoform <- select(dataset, Sample, "9a", "9b")

## Lets normalize by the M of uniquely aligned reads

dataset_gene_normalized <- dataset_gene %>% 
  mutate(across(where(is.numeric), ~ .x/depth$`M Aligned`)) # value/M of reads

## Lets merge with metadata
dataset_isoform <- dataset_isoform %>%
  separate(Sample, "Sample", "_", extra="drop")

dataset_isoform <- inner_join(metadata, dataset_isoform, by ="Sample")
## Lets factorize the values
#dataset_isoform$Time_point <- as.factor(dataset_isoform$Time_point)
dataset_isoform$Batch <- as.factor(dataset_isoform$Batch)

 # mutate("Package" = as.factor("mosdepth")) 



## Let's merge the sequencing depth for normalization

dataset <- inner_join(dataset, depth, by="Sample")


## remove the last part of the string on the sample names
dataset$Sample<-gsub("_Homo_sapiens_RNA-Seq","",as.character(dataset$Sample)) 
dataset$Sample<-gsub("_Gorilla_gorilla_RNA-Seq","",as.character(dataset$Sample)) 
